...
public:
	...
	virtual void on_send(const char *, size_t) = 0;
	virtual void on_recv(const char *, size_t) = 0;

private:
	void dispatch_send_(const char *, size_t);
	void dispatch_recv();
	void start_send_(const char *, size_t);
	void start_recv_();
	...
	char buf_recv_[buf_size_];
	char buf_send_[buf_size_];
	bool chunk_to_send_;
